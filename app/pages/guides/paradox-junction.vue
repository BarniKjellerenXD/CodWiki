      ...
      <section class="min-w-0">
        <div class="rounded-2xl bg-white/10 border border-white/20 backdrop-blur-md shadow-xl shadow-cyan-500/10 p-4 sm:p-6">
          <header class="flex items-center justify-between gap-4 mb-4 border-b border-white/10 pb-3">
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-fuchsia-300">Paradox Junction</h1>
...
const pinnedOrderDefault = [
  'wiki_relics',
  'wiki_blundergat',
  'wiki_sundergat_upgrade',
  'wiki_main_quest',
  'wiki_the_dark_heart_boss_fight',
]
...
function loadPins() {
  try {
    const raw = localStorage.getItem('guide-pins-paradox')
    if (raw) {
...
function savePins() { localStorage.setItem('guide-pins-paradox', JSON.stringify(pins.value)) }
...
onMounted(async () => {
  try {
    const res = await fetch('/guides/paradox-junction.html', { cache: 'no-store' })
